...
// Default: lock configuration (no provider/model picker UI)
...
// Always set up user config from env (UI can still be locked via CAN_CHANGE_KEYS=false)
...
const startServers = async () => {
  ...
  const shouldStartOllamaServer = () => {
    if ((process.env.DISABLE_OLLAMA_SERVER || "").toLowerCase() === "true") {
      return false;
    }
    return (process.env.LLM || "").toLowerCase() === "ollama";
  };

  let ollamaProcess = null;
  if (shouldStartOllamaServer()) {
  ...
  // Keep the Node process alive until one of the processes exits
  const exitPromises = [
