...
const presentationGenerationSlice = createSlice({
  ...
  reducers: {
    ...
    // Set presentation data (с нормализацией слайдов — без битых записей)
    setPresentationData: (state, action: PayloadAction<PresentationData>) => {
      const normalized = normalizePresentationData(action.payload);
      if (normalized) {
        state.presentationData = normalized as PresentationData;
