...
export function convertElementAttributesToPptxSlides(
  slidesAttributes: SlideAttributesResult[] | null | undefined
): PptxSlideModel[] {
  if (!Array.isArray(slidesAttributes)) {
    return [];
  }

  return slidesAttributes
    .filter((sa): sa is SlideAttributesResult => sa != null)
    .map((slideAttributes) => {
    const elements = slideAttributes.elements ?? [];
...
function convertToAutoShapeBox(element: ElementAttributes): PptxAutoShapeBoxModel {
  ...
  return {
    ...
    position,
    text_wrap: element.textWrap ?? true,
    border_radius: borderRadius !== undefined ? Math.round(borderRadius) : undefined,
    paragraphs
  };
}
