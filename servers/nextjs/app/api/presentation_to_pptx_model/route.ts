...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Puppeteer runs server-side: use internal URL (Next.js on 3000).
    // External origin (e.g. localhost:5000) is unreachable from inside Docker.
    const port = process.env.PORT || "3000";
    const baseUrl = process.env.PDF_MAKER_BASE_URL || `http://127.0.0.1:${port}`;
    [browser, page] = await getBrowserAndPage(baseUrl, id);
...
async function getBrowserAndPage(
  ...
  await page.goto(`${origin}/pdf-maker?id=${encodeURIComponent(id)}`, {
    waitUntil: "networkidle0",
    timeout: 300000,
  });
  // Wait for slides to load (API fetch is async; networkidle0 may fire before data arrives)
  await page.waitForFunction(
    () => {
      const wrapper = document.getElementById("presentation-slides-wrapper");
      if (!wrapper) return false;
      const slides = wrapper.querySelectorAll(":scope > div[data-speaker-note]");
