...
const ImageEditor = ({
  ...
  const getPreviousGeneratedImage = async () => {
    try {
      ...
      toast.error(t(language, "failedToGetPreviousGeneratedImages"));
      console.error("error in getting previous generated images", error);
      setError(
        error.message ||
          t(language, "failedToGetPreviousGeneratedImages")
      );
    }
  };
  ...
  const handleGenerateImage = async () => {
    if (!prompt) {
      setError(t(language, "pleaseEnterPrompt"));
      return;
    }
    try {
      ...
      setError(err.message || t(language, "failedToGenerateImage"));
    } finally {
  ...
  const handleFileUpload = async (
    ...
    if (file.size > 5 * 1024 * 1024) {
      setUploadError(t(language, "fileSizeLessThan5mb"));
      return;
    }

    // Validate file type
    if (!file.type.startsWith("image/")) {
      setUploadError(t(language, "pleaseUploadImageFile"));
      return;
    }
    try {
      setIsUploading(true);
      ...
      setUploadError(t(language, "failedToUploadImage"));
      toast.error(err.message || t(language, "failedToUploadImage"));
  ...
  const getUploadedImages = async () => {
    try {
      ...
      toast.error(err.message || t(language, "failedToGetUploadedImages"));
  ...
  const handleDeleteImage = async (image_id: string) => {
    try {
      ...
      toast.success(result.message || t(language, "imageDeletedSuccessfully"));
    } catch (err:any) {
      toast.error(err.message || t(language, "failedToDeleteImage"));
    }
  };
  return (
    <div className="image-editor-container">
      <Sheet open={isOpen} onOpenChange={() => handleClose()}>
        ...
        >
          <SheetHeader>
            <SheetTitle>{t(language, "updateImage")}</SheetTitle>
          </SheetHeader>
            ...
            <Tabs defaultValue="generate" className="w-full" onValueChange={handleTabChange}>
              ...
              <TabsContent value="generate" className="mt-4 space-y-4 overflow-y-auto hide-scrollbar h-[85vh]">
                <div className="space-y-4">
                  <div>
                    <h3 className="text-sm font-medium mb-1">{t(language, "currentPrompt")}</h3>
                  ...
                  <div>
                    ...
                    <Textarea
                      placeholder={t(language, "describeImagePlaceholder")}
                      value={prompt}
                  ...
                  >
                    ...
                    {isGenerating ? t(language, "generating") : t(language, "generateImage")}
                  </Button>
                  ...
                  {previousGeneratedImages.length > 0 && (
                    <div className="mt-4">
                      <h3 className="text-sm font-medium mb-2">
                        {t(language, "previousGeneratedImages")}
                      </h3>
                      <div className="grid grid-cols-2 gap-4  ">
              ...
              <TabsContent value="upload" className="mt-4 space-y-4">
                <div className="space-y-4">
                  ...
                  >
                    ...
                    >
                      ...
                      <span className="text-sm text-gray-600">
                        {isUploading
                          ? t(language, "uploadingYourImage")
                          : t(language, "clickToUploadImage")}
                      </span>
                      <span className="text-xs text-gray-500 mt-1">
                        {t(language, "maxFileSize")}
                      </span>
                    </label>
                  </div>
                  ...
                  {(uploadedImageUrl || isUploading) && (
                    <div className="mt-4">
                      <h3 className="text-sm font-medium mb-2">
                        {t(language, "uploadedImagePreview")}
                      </h3>
                  ...
                  <div>
                    <h3 className="text-sm font-medium mb-2">{t(language, "uploadedImagesLabel")}</h3>
                    <div className="grid grid-cols-2 gap-4">
