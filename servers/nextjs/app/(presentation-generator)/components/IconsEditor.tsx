...
const IconsEditor = ({
  ...
  const handleIconSearch = async () => {
    ...
    try {
      ...
      toast.error(error.message || t(language, "failedToFetchIcons"));
      setIcons([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="icons-editor-container">
      ...
      <Sheet open={isOpen} onOpenChange={() => handleClose()}>
        ...
        >
          <SheetHeader>
            <SheetTitle>{t(language, "chooseIcon")}</SheetTitle>
          </SheetHeader>
          ...
          <div className="mt-6 space-y-4">
            ...
            >
              <div className="relative mb-3">
                ...
                <Input
                  placeholder={t(language, "searchIconsPlaceholder")}
                  value={searchQuery}
              ...
              <Button
                type="submit"
                variant="outline"
                className="w-full text-semibold text-primary"
                onClick={(e) => e.stopPropagation()}
              >
            ...
            <div className="max-h-[80vh] hide-scrollbar overflow-y-auto p-1">
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center w-full h-[60vh] text-center text-gray-500 space-y-4">
                  ...
                  <p className="text-sm">{t(language, "noIconsFound")}</p>
                  <p className="text-xs">{t(language, "refineSearch")}</p>
                </div>
              )}
            </div>
          </div>
