...
const SupportingDoc = ({ files, onFilesChange }: SupportingDocProps) => {
    ...
    const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
        ...
        if (invalidFiles.length > 0) {
            toast.error(t(language, "invalidFileTypeTitle"), {
                description: t(language, "invalidFileTypeBody"),
            });
            return;
        }

        if (hasPdf && droppedFiles.some(file => file.type === 'application/pdf')) {
            toast.error(t(language, "multiplePdfTitle"), {
                description: t(language, "multiplePdfBody"),
            });
            return;
        }
        ...
        if (validFiles.length > 0) {
            ...
            toast.success(t(language, "filesSelectedTitle"), {
                description: t(language, "filesSelectedBody", { count: validFiles.length }),
            })
        }
    }
    ...
    return (
        <div className="w-full">
            <h2 className="text-[#444] font-instrument_sans pt-4 text-lg mb-4">
                {t(language, "supportingDocuments")}
            </h2>
            <div
            ...
            >
                <div className="flex-1 flex flex-col items-center justify-center p-6">
                    ...
                    <p className="text-gray-600 text-center mb-2">
                        {isDragging
                            ? t(language, "dropFileHere")
                            : t(language, "dragDropOrClick")
                        }
                    </p>
                    <p className="text-gray-400 text-sm text-center mb-4">
                        {t(language, "supportsFormats")}
                    </p>
                        ...
                        <div className="p-4">
                            <div className="flex items-center justify-between mb-3">
                                <h3 className="text-sm font-medium text-gray-700">
                                    {t(language, "selectedFiles", { count: files.length })}
                                </h3>
                            </div>
                                            ...
                                            transition-colors flex items-center justify-center relative"
                                                ...
                                                <div className="p-3 relative">
                                                    <p className="text-sm font-medium text-gray-700 truncate mb-1 pr-2">
                                                        {file.name || t(language, "unnamedFile")}
                                                    </p>
