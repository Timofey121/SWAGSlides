...
import { normalizePresentationData } from "../../../../utils/slideNormalizer";
        ...
        switch (data.type) {
          ...
          case "complete":
            try {
              ...
              const normalized = pres ? normalizePresentationData(pres) : null;
              dispatch(setPresentationData((normalized || pres) as any));
          ...
          case "closing":
            const closingPres = data.presentation;
            const closingNormalized = closingPres ? normalizePresentationData(closingPres) : null;
            dispatch(setPresentationData((closingNormalized || closingPres) as any));
            setLoading(false);
            dispatch(setStreaming(false));
            eventSource.close();
