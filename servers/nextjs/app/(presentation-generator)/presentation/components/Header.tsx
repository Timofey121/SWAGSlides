...
const Header = ({
  ...
  const get_presentation_pptx_model = async (id: string): Promise<PptxPresentationModel> => {
    ...
    if (!response.ok) {
      const detail =
        ...
        "Failed to build PPTX model";
      throw new Error(detail);
    }
    return json as PptxPresentationModel;
  };
  ...
  const handleExportPptx = async () => {
    ...
    try {
      ...
      setShowLoader(false);
      const errMsg = error?.message || t(config?.language, "exportTroubleBody");
      toast.error(t(config?.language, "exportTroubleTitle"), {
        description: errMsg,
      });
    } finally {
      setShowLoader(false);
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      toast.error(t(config?.language, "exportTroubleTitle"), {
        description:
          t(config?.language, "exportTroubleBody"),
      });
    } finally {
      setShowLoader(false);
    }
  ...
  const MenuItems = ({ mobile }: { mobile: boolean }) => (
    <div className="flex flex-col lg:flex-row items-center gap-4">
      <LangSelect
        value={(config?.language as string) || LanguageType.Russian}
        onValueChange={async (value) => {
          ...
          // Make the language change visible immediately on the page
          // by regenerating slide content in the selected language.
          dispatch(clearPresentationData());
          dispatch(clearHistory());
        ...
        name="language"
      >
        <LangSelectTrigger className="w-[160px] bg-white/15 text-white border-white/30 focus-visible:ring-white/30">
          <LangSelectValue placeholder={t(config?.language, "language")} />
        </LangSelectTrigger>
        <LangSelectContent>
          <LangSelectItem value={LanguageType.Russian}>Русский</LangSelectItem>
          <LangSelectItem value={LanguageType.English}>English</LangSelectItem>
        </LangSelectContent>
      </LangSelect>
      {/* undo redo */}
      <button onClick={handleReGenerate} disabled={isStreaming || !presentationData} className="text-white  disabled:opacity-50" >

        {t(config?.language, "regenerate")}
      </button>
      <div className="flex items-center gap-2 ">
        <ToolTip content={t(config?.language, "undo")}>
        ...
        <ToolTip content={t(config?.language, "redo")}>
      ...
      <div style={{
        ...
        <Popover open={open} onOpenChange={setOpen} >
          <PopoverTrigger asChild>
            <Button className={`border py-5 text-primary font-bold rounded-[32px] transition-all duration-500 hover:border hover:bg-primary hover:text-primary-foreground w-full ${mobile ? "" : "bg-white"}`}>
              <SquareArrowOutUpRight className="w-4 h-4 mr-1" />
              {t(config?.language, "export")}
            </Button>
          </PopoverTrigger>
  ...
  return (
    <>
      <OverlayLoader
        show={showLoader}
        text={t(config?.language, "exportingPresentation")}
        showProgress={true}
        duration={40}
      />
      <div
        ...
        <Wrapper className="flex items-center justify-between py-1">
          <Link href="/upload" className="min-w-[162px]">
