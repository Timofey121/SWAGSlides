...
const SlideContent = ({ slide, index, presentationId }: SlideContentProps) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!value?.trim()) {
      toast.error(t(language, "enterPromptBeforeSubmitting"));
      return;
    }
    setIsUpdating(true);
    ...
    try {
      ...
      if (response) {
        ...
        toast.success(t(language, "slideUpdatedSuccessfully"));
      }
    } catch (error: any) {
      console.error("Error in slide editing:", error);
      toast.error(t(language, "slideEditingError"), {
        description: error.message || t(language, "slideEditingError"),
      });
    } finally {
      setIsUpdating(false);
    }
  ...
  const onDeleteSlide = async () => {
    try {
      ...
      toast.error(t(language, "slideDeletingError"), {
        description: error.message || t(language, "slideDeletingError"),
      });
    }
  };
      ...
      >
        ...
        >
          ...
          {showNewSlideSelection && (
            <NewSlide
              index={index}
              templateID={`${slide.layout?.split(":")[0] ?? "default"}`}
          ...
          {!isStreaming && (
            <ToolTip content={t(language, "deleteSlide")}>
              <div
                onClick={() => {
          ...
          {!isStreaming && (
            <div className="absolute top-2 z-20 sm:top-4 hidden md:block left-2 sm:left-4 transition-transform">
              <Popover>
                <PopoverTrigger>
                  <ToolTip content={t(language, "updateSlideUsingPrompt")}>
                    <div
                      className="p-2 group-hover:scale-105 rounded-lg bg-primary hover:bg-primary/90 hover:shadow-md transition-all duration-300 cursor-pointer shadow-md"
                    >
                  ...
                  <div className="space-y-4">
                    ...
                    >
                      <Textarea
                        ...
                        placeholder={t(language, "enterPromptHere")}
                      ...
                      <button
                        disabled={isUpdating}
                        type="submit"
                        className={`bg-primary hover:bg-primary/90 rounded-[32px] px-4 py-2 text-primary-foreground flex items-center justify-end gap-2 ml-auto ${isUpdating ? "opacity-70 cursor-not-allowed" : ""
                          }`}
                        onClick={() => {
                          trackEvent(MixpanelEvent.Slide_Update_From_Prompt_Button_Clicked, { pathname });
                        }}
                      >
                        {isUpdating ? t(language, "updating") : t(language, "update")}
