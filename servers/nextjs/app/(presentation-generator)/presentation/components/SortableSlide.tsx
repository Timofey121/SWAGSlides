...
export function SortableSlide({ slide, index, selectedSlide, onSlideClick }: SortableSlideProps) {
    ...
    const style = {
        ...
        borderColor: selectedSlide === index ? `hsl(var(--primary))` : `var(--stroke, #e5e7eb)`
    };
    ...
    return (
        <div
            ...
            {...listeners}
            onClick={handleClick}
            className={` cursor-pointer border-[3px] relative  p-1 shadow-lg   rounded-md transition-all duration-200 ${selectedSlide === index ? ' border-primary' : 'border-gray-300'
                }`}
        >
