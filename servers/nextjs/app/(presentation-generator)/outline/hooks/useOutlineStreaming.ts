    ...
    const initializeStream = async () => {
      ...
      try {
          ...
          switch (data.type) {
            ...
            case "complete":
              ...
              try {
                ...
                toast.error(t(language, "failedToParsePresentationData"));
                eventSource.close();
              }
              accumulatedChunks = "";
              break;
            ...
            case "error":
              ...
              highestIndexRef.current = -1;
              eventSource.close();
              toast.error(t(language, "outlineStreamingErrorTitle"),
                {
                  description: data.detail || t(language, "failedToConnectServer"),
                }
              );
              break;
          }
        ...
        eventSource.onerror = () => {
          ...
          highestIndexRef.current = -1;
          eventSource.close();
          toast.error(t(language, "failedToConnectServer"));
        };
      } catch (error) {
        ...
        activeIndexRef.current = -1;
        highestIndexRef.current = -1;
        toast.error(t(language, "failedToInitializeConnection"));
      }
    };
    initializeStream();
    return () => {
