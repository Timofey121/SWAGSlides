...
const GenerateButton: React.FC<GenerateButtonProps> = ({
  ...
  const getButtonText = () => {
    ...
    if (streamState.isLoading || streamState.isStreaming) return t(language, "loading");
    if (!selectedTemplate) return t(language, "selectTemplate");
    return t(language, "generatePresentation");
  };
  ...
  return (
    <Button
      ...
      className="bg-primary w-full rounded-lg text-base sm:text-lg py-4 sm:py-6 font-instrument_sans font-semibold hover:bg-primary/90 text-primary-foreground disabled:opacity-50 disabled:cursor-not-allowed"
    >
      <svg
        className="mr-2"
        width="24"
