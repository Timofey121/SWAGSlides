...
import { buildAudienceHardInstructions, clampSlidesForAudience } from "../../upload/audienceRules";
import { AudienceType } from "../../upload/type";
import { t } from "../../i18n";
...
const DocumentsPreviewPage: React.FC = () => {
  ...
  const maintainDocumentTexts = async () => {
    ...
    if (promises.length > 0) {
      ...
      try {
        ...
        toast.error(t(language, "failedToReadDocument"));
      }
  ...
  const handleCreatePresentation = async () => {
    try {
      setShowLoading({
        message: t(language, "generatingOutline"),
        show: true,
        duration: 40,
        progress: true,
      });
      ...
      const audience = (config?.audience as AudienceType) || AudienceType.Consulting;
      const slides = clampSlidesForAudience(audience, config?.slides);
      const slidesN = Number(slides);
      const autoToc =
        audience !== AudienceType.AgencyColdKP &&
        Number.isFinite(slidesN) &&
        slidesN > 20;
      const hardRules = buildAudienceHardInstructions(audience, slides);
      const extra = (config?.instructions || "").trim();
      const effectiveInstructions =
        extra.length > 0
          ? `${hardRules}\n\n# Additional user instructions\n${extra}`
          : hardRules;
       const nSlides = slides ? Math.max(1, parseInt(slides, 10) || 1) : 10;
       const createResponse = await PresentationGenerationApi.createPresentation(
        {
          ...
          include_table_of_contents: autoToc ? true : !!config?.includeTableOfContents,
      ...
      toast.error(t(language, "error"), {
        description: error.message || t(language, "createPresentationError"),
      });
      setShowLoading({
        message: t(language, "createPresentationError"),
        show: true,
        duration: 10,
        progress: false,
      });
      ...
      <div className="h-full mr-4">
        <div className="overflow-y-auto custom_scrollbar h-full">
          <div className="h-full w-full max-w-full flex flex-col mb-5">
            <h1 className="text-2xl font-medium mb-5">{t(language, "content")}</h1>
        ...
        {documentKeys.length > 0 && (
          <div className="mt-8">
            <p className="text-xs mt-2 text-[#2E2E2E] opacity-70">
              {t(language, "documents").toUpperCase()}
            </p>
      ...
      <div className="flex mt-6 gap-4 font-instrument_sans">
        {!isOpen && (
          <div className="fixed left-4 top-1/2 -translate-y-1/2 z-50">
            <ToolTip content={t(language, "openPanel")}>
              <Button
                onClick={() => setIsOpen(true)}
                className="bg-primary text-primary-foreground p-3 shadow-lg hover:bg-primary/90"
              >
        ...
        <div className="fixed bottom-5 right-5">
          <Button
            ...
            className="flex items-center gap-2 px-8 py-6 rounded-sm text-md bg-primary text-primary-foreground hover:bg-primary/90"
          >
            <span className="text-white font-semibold">{t(language, "next")}</span>
            <ChevronRight />
          </Button>
        </div>
      </div>
