...
def get_system_prompt(
    ...
    return f"""
        ...
        - CRITICAL: Generate ALL slide content (title, body, speaker note, image prompts, icon queries) ONLY in the language specified in "Slide Content Language". Never use Chinese, Japanese, Arabic, or any other language unless explicitly requested. If language is "Russian" or "English", output must be exclusively in that language.
        ...
        - CRITICAL: Never cause text overlap or overflow. If the slide has many text blocks or limited space, generate shorter content. When in doubt, make text smaller/shorter—this overrides verbosity rules. Prefer less content over overlapping text.
        - CRITICAL: Every bullet point, description, and text block must be COMPLETE—never cut off mid-sentence or mid-word. If space is limited, use fewer items with shorter text rather than truncating. Full text must fit on the slide.
