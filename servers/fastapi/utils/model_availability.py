...
from utils.set_env import set_openai_model_env
...
_PREFERRED_OPENAI_MODELS = [
    "gpt-4.1",
]


def _pick_best_available_model(available_models: list[str]) -> str | None:
    if not available_models:
        return None
    for m in _PREFERRED_OPENAI_MODELS:
...
async def check_llm_and_image_provider_api_or_model_availability():
    ...
    if not can_change_keys:
        if get_llm_provider() == LLMProvider.OPENAI:
            ...
            # If model isn't set or isn't available for this key, pick the best available.
            if not openai_model or openai_model not in available_models:
                best_model = _pick_best_available_model(available_models)
                if not best_model:
                    raise Exception("No OpenAI models available for provided API key")
                set_openai_model_env(best_model)
