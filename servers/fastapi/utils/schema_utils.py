...
def ensure_strict_json_schema(
    ...
    *,
    ...
    # arrays
    # { 'type': 'array', 'items': {...} } or type: ["array", "null"] or prefixItems (tuple)
    # OpenAI requires array schemas to have 'items' - add default if missing
    is_array = (
        typ == "array"
        or (isinstance(typ, list) and "array" in typ)
        or "prefixItems" in json_schema
    )
    if is_array:
        items = json_schema.get("items")
        if not isinstance(items, dict):
            # Tuple/fixed arrays from Zod may lack items; add string items as safe default
            json_schema["items"] = ensure_strict_json_schema(
                {"type": "string"}, path=(*path, "items"), root=root
            )
        else:
            json_schema["items"] = ensure_strict_json_schema(
