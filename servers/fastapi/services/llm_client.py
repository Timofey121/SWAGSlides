...
class LLMClient:
    ...
    def _get_google_messages(self, messages: List[LLMMessage]) -> List[Any]:
        try:
    ...
    async def _generate_google(
        ...
        try:
            from google.genai.types import GenerateContentConfig, Tool as GoogleTool
        except ImportError as e:
            raise HTTPException(
                status_code=400,
                detail="Google provider selected but 'google-genai' package is not installed",
    ...
    async def _generate_openai_structured(
        ...
        # Always normalize schema for OpenAI - it rejects invalid schemas (e.g. array missing items)
    ...
    async def _search_google(self, query: str) -> str:
        try:
            from google.genai.types import GenerateContentConfig, GoogleSearch, Tool as GoogleTool
        except ImportError as e:
            raise HTTPException(
                status_code=400,
                detail="Google provider selected but 'google-genai' package is not installed",
