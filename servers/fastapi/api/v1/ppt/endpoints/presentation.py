...
async def create_presentation(
    ...
    if n_slides < 1:
        raise HTTPException(
            status_code=400,
            detail="Number of slides must be at least 1",
        )
...
async def update_presentation(
    ...
    if language:
        presentation_update_dict["language"] = language

    if n_slides or title or language:
...
async def export_presentation_as_pptx(
    ...
    if not pptx_model.slides:
        raise HTTPException(
            status_code=400,
            detail="Presentation has no slides. Ensure the presentation is fully loaded before exporting.",
        )
...
async def check_if_api_request_is_valid(
    ...
    if request.template == "agency-kp":
        # Standard agency cold proposal is 3–6 slides total
        if request.n_slides < 3 or request.n_slides > 6:
            raise HTTPException(
                status_code=400,
                detail="For template 'agency-kp' number of slides must be between 3 and 6 (inclusive).",
            )
